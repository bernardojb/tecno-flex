...
# --------------------------------------------------
    ...
    $sql          = "SELECT prod_id, prod_url, prod_title FROM products ORDER BY prod_id DESC LIMIT 1";
    $arrayParam   = array();
    ...
    $id_produto = $data_prod->prod_id;
    $url_prod   = $data_prod->prod_url;

    $fp = fopen("../.htaccess", "a+");

    #RewriteRule ^prod-prod-teste/?$ index.php?produto_final [NC,L]

    if ($id_cat == '1'){ 
        $escreve = fwrite($fp, "\n RewriteRule ^$url_prod/?$ index.php?src=produto_final&id=$id_produto [NC,L] ");
    }else{
        $escreve = fwrite($fp, "\n RewriteRule ^$url_prod/?$ index.php?src=produto_final&id=$id_produto [NC,L] ");
    }
